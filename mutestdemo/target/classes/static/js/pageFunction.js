...
function tAll() {
  ...
  if (document.getElementById("toggle-t-all").checked) {
    for (var i = 0; i < tOpNum; i++) ops[i].checked = true;
  } else for (var i = 0; i < tOpNum; i++) ops[i].checked = false;
}
function eAll() {
  var ops = document.getElementsByClassName("typeCheck");
  if (document.getElementById("toggle-e-all").checked) {
    for (var i = tOpNum; i < tOpNum + eOpNum; i++) ops[i].checked = true;
  } else for (var i = tOpNum; i < tOpNum + eOpNum; i++) ops[i].checked = false;
}
...
function clearCount(type) {
  ...
  if (type == 0) for (var i = 0; i < 9; i++) items[i].innerHTML = "0";
  else if (type == 1) for (var i = 9; i < 24; i++) items[i].innerHTML = "0";
  else if (type == -1)
    for (var i = 0; i < items.length; i++) items[i].innerHTML = "0";
}
function showChooseMutantSpan(type, conNum) {
  ...
  if (type == 0) {
    ...
    for (var i = 0; i < mutants[type][conNum].mutateLine.length; i++) {
      spanText +=
        "<div>\n" +
        '          <input id="chooseRadio-t-' +
        i +
        '" class="chooseRadio-t" type="radio" onchange=\'changeMuText(0,' +
        conNum +
        "," +
        i +
        ')\' name="item" value=""';
      ...
      spanText +=
        ">\n" +
        '          <label for="chooseRadio-t-' +
        i +
        '"></label>\n' +
        '          <span style="margin-left: 10px">' +
        mutants[type][conNum]["mutateLineType"][i] +
        "_" +
        (mutants[type][conNum]["mutateLineNums"][i] + 1) +
        "</span>\n" +
        "        </div>";
      var num = parseInt(
        document.getElementById(
          mutants[type][conNum]["mutateLineType"][i] + "MuCount"
        ).innerText
      );
      num += 1;
      $("#" + mutants[type][conNum]["mutateLineType"][i] + "MuCount").html(
        "" + num
      );
    }
    ...
    for (var i = 0; i < mutants[type][conNum].mutateLine.length; i++) {
      spanText +=
        "<div>\n" +
        '          <input id="chooseRadio-e-' +
        i +
        '" class="chooseRadio-t" type="radio" onchange=\'changeMuText(1,' +
        conNum +
        "," +
        i +
        ')\' name="item" value=""';
      ...
      spanText +=
        ">\n" +
        '          <label for="chooseRadio-e-' +
        i +
        '"></label>\n' +
        '          <span style="margin-left: 10px">' +
        mutants[type][conNum]["mutateLineType"][i] +
        "_" +
        mutants[type][conNum]["mutateLineNums"][i] +
        "</span>\n" +
        "        </div>";
      var num = parseInt(
        document.getElementById(
...
function showMuText(type, conNum, muNum) {
  ...
  oriText +=
    ' <font color="red">' +
    mutants[type][conNum]["oriLine"][muLineNum] +
    "</font><br>";
  ...
  for (
    var i = muLineNum + 1;
    i < mutants[type][conNum]["oriLine"].length;
    i++
  ) {
    oriText += '<div class="lineNum">' + no + "</div>";
    oriText += mutants[type][conNum]["oriLine"][i] + "<br>";
...
function getTestProjectFolder() {
  ...
  $.ajax({
    ...
    success: function (result) {
      document
        .getElementById("loadTestProject")
        .setAttribute("hidden", "hidden");
      if (
        result.toString().length > 4 &&
        result.toString().substr(0, 4) == "Fail"
      ) {
        projectPath = "";
        $("#project-path-input").removeAttr("disabled");
        ...
        for (var i = 1; i < result.length; i++) {
          ...
          var text =
            '<div class="col-md-3"class="contractFile"style=\'margin-bottom: 15px;max-width:90%;margin-top: 15px\'>\n' +
            '                                        <div class="toggle-switch" data-ts-color="info">\n' +
            '                                            <input class="contractFileChoose"id="ts' +
            contractsName.length +
            ...
            '                                            <label for="ts' +
            contractsName.length +
            '" class="contract-label" style="margin-left: 20px">' +
            result[i] +
            "</label>\n" +
            "                                        </div>\n" +
            "                                    </div>";
  ...
  for (var j = 0; j < mutants[0].length; j++) {
    for (var m = 0; m < mutants[0][j]["mutateLineType"].length; m++) {
      var num =
        parseInt(
          document.getElementById(
            mutants[0][j]["mutateLineType"][m] + "-MuCount"
          ).innerText
        ) + 1;
      ...
      for (var j = 0; j < mutants[i].length; j++) {
        for (var m = 0; m < mutants[i][j]["mutateLineType"].length; m++) {
          var num =
            parseInt(
              document.getElementById(
                mutants[i][j]["mutateLineType"][m] + "-MuCount"
              ).innerText
            ) + 1;
          $("#" + mutants[i][j]["mutateLineType"][m] + "-MuCount").html(
            "" + num
          );
        }
      }
...
function showChooseMutantSpan0(conNum) {
  ...
  for (var i = 0; i < mutants[0][conNum].mutateLine.length; i++) {
    spanText +=
      "<div>\n" +
      '          <input id="chooseRadio-' +
      i +
      '" class="chooseRadio" type="radio" onchange=\'changeMuText0(' +
      conNum +
      "," +
      i +
      ')\' name="item" value=""';
    ...
    spanText +=
      ">\n" +
      '          <label for="chooseRadio-' +
      i +
      '"></label>\n' +
      '          <span style="margin-left: 10px">' +
      mutants[0][conNum]["mutateLineType"][i] +
      "_" +
      (mutants[0][conNum]["mutateLineNums"][i] + 1) +
      "</span>\n" +
      "        </div>";
    var num = parseInt(
      document.getElementById(
        mutants[0][conNum]["mutateLineType"][i] + "MuCount"
      ).innerText
    );
    num += 1;
...
function showMuText0(conNum, muNum) {
  ...
  oriText +=
    ' <font color="red">' +
    mutants[0][conNum]["oriLine"][muLineNum] +
    "</font><br>";
  ...
  if (isTesting) {
    $.ajax({
      ...
      success: function (result) {
        ...
        $("#loadMuTestText").html(
          "Test the mutants now...(" + result + "/" + mutantsNum + ")"
        );
  ...
  if (mutants != null && mutants[0].length > 0) {
    ...
    $.ajax({
      ...
      success: function (result) {
        isTesting = false;
        document
          .getElementById("loadMutationTest")
          .setAttribute("hidden", "hidden");
        if (
          result.toString().length > 4 &&
          result.toString().substr(0, 4) == "Fail"
          ...
          $("#teScore").html(
            parseInt(
              ((result[0]["kill"] + result[1]["kill"]) * 100) /
    ...
    $("#loadMuTestText").html(
      "Test the mutants now...(0" + "/" + mutantsNum + ")"
    );
    getTestedNum();
  } else {
...
function exportReport() {
  $(this).openWindow(
    "Attention",
    "Report has been export to <MuSC_Report> folder under your truffle project folder! ",
    '["OK"]'
  );
}
